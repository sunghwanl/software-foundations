...
Proof.
  ...
  - intros st. exists (t_update st x (aeval st a)). constructor. reflexivity.
  - intros st.
    destruct (IHno_whilesR1 st) as [st' H1].
    destruct (IHno_whilesR2 st') as [st'' H2].
    exists st''. apply (E_Seq c1 c2 st st' st'').
    apply H1. apply H2.
  - intros st.
    destruct (beval st b) eqn:Hb.
    + destruct (IHno_whilesR1 st) as [st' H1].
      exists st'. apply (E_IfTrue st st' b ct cf Hb). apply H1.
    + destruct (IHno_whilesR2 st) as [st' H2].
      exists st'. apply (E_IfFalse st st' b ct cf Hb). apply H2.
Qed.                                      
(** [] *)
...
Theorem s_compile_app : forall st e stack prog,
                          s_execute st stack ((s_compile e) ++ prog)
                          = s_execute st ((aeval st e) :: stack) prog.
Proof.
  intros st e.
  induction e; try reflexivity;
  simpl; intros; rewrite <- app_assoc; rewrite <- app_assoc;
...
Proof.
  ...
  rewrite -> s_compile_app; rewrite -> s_compile_app; reflexivity.
Qed.
(** [] *)
...
Inductive ceval : com -> state -> status -> state -> Prop :=
  ...
  | E_Ass : forall st x e n,
              aeval st e = n ->
              (CAss x e) / st \\ SContinue / (t_update st x n)
  | E_IfTrue : forall st b c1 c2 status st',
                 beval st b = true ->
                 c1 / st \\ status / st' ->
                 (IFB b THEN c1 ELSE c2 FI) / st \\ status / st'
  | E_IfFalse : forall st b c1 c2 status st',
                  beval st b = false ->
                  c2 / st \\ status / st' ->
                  (IFB b THEN c1 ELSE c2 FI) / st \\ status / st'
  | E_SeqContinue : forall st st' st'' c1 c2 status,
                      c1 / st \\ SContinue / st' ->
                      c2 / st' \\ status / st'' ->
                      (c1 ;; c2) / st \\ status / st''
  ...
  | E_WhileLoopContinue : forall st st' st'' b c status,
                            beval st b = true ->
                            c / st \\ SContinue / st' ->
                            (WHILE b DO c END) / st' \\ status / st'' ->
                            (WHILE b DO c END) / st \\ SContinue / st''
...
Proof.
  ...
  induction H1; inversion Heqcom.
  - rewrite -> H1 in H. rewrite -> H in H2. inversion H2.
  - apply IHceval2. rewrite H1. rewrite H3. reflexivity. apply H2.
  - rewrite H4 in H1. exists st. apply H1.
Qed.
...
Proof.
  ...
  try (apply IHceval in H9; assumption);
  try (rewrite H in H8; inversion H8);
  try (rewrite H3 in H; inversion H).
  - split.
    + rewrite H in H6. rewrite H6. reflexivity.
    + reflexivity.
  - apply IHceval1 in H1. destruct H1 as [Hst Hsc].
    rewrite <- Hst in H6. apply IHceval2 in H6. apply H6.
  - apply IHceval1 in H5. destruct H5 as [_ contra]. inversion contra.
  - apply IHceval in H3. destruct H3 as [_ contra]. inversion contra.
  - apply IHceval in H6. apply H6.
  - rewrite H5 in H. rewrite H6 in H. inversion H.
  - apply IHceval1 in H4. destruct H4 as [Hst _].
    rewrite <- Hst in H8. apply IHceval2 in H8. destruct H8 as [Hst' _].
    split. apply Hst'. reflexivity.
  - apply IHceval1 in H7. destruct H7 as [_ contra]. inversion contra.
  - rewrite <- H6 in H7. rewrite -> H7 in H. inversion H.
  - apply IHceval in H5. destruct H5 as [_ contra]. inversion contra.
  - apply IHceval in H8. destruct H8 as [Hst _].
    split. apply Hst. reflexivity.
Qed.
