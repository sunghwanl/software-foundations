...
Proof.
  ...
  - intros E'. induction E'.
    ...
    + apply ev_sum. apply IHE'1. apply IHE'2.
  - intros E. induction E.
    + apply ev'_0.
    + assert (SS: S (S n) = 2 + n).
      { reflexivity. }
...
Proof.
  ...
  assert (H': ev (n + n)).
  { apply ev_even_iff.
    exists n. symmetry. apply double_plus.
  }
...
| tr_00 : total_relation 0 0
| tr_S1 : forall m n, total_relation m n -> total_relation (S m) n
| tr_S2 : forall m n, total_relation m n -> total_relation m (S n).
(** [] *)
...
Proof.
  ...
  - unfold fR. intros H. induction H.
    ...
    +  simpl in IHR.
      rewrite -> plus_comm in IHR. simpl in IHR.
      inversion IHR.
...
| s1 : subseq [] []
| s2 : forall l1 l2, subseq l1 l2 -> forall n, subseq l1 (n::l2)
| s3 : forall l1 l2, subseq l1 l2 -> forall n, subseq (n::l1) (n::l2).
...
Lemma reduce : forall X (l1 l2 : list X) (n : X),
                 subseq (n :: l1) l2 -> subseq l1 l2.
Proof.
  intros X l1 l2 n H.
  induction l2 as [| n' l2' IH].
  - inversion H.
  - inversion H.
    + apply IH in H2.
      apply s2. apply H2.
...
Proof.
  ...
  induction H2.
  - intros l1 H. apply H.
  - intros l0 H. apply IHsubseq in H.
    apply s2. apply H.
  - intros l0 H.
    inversion H.
    + apply s2. apply IHsubseq in H3. apply H3.
    + apply s3. apply IHsubseq in H3. apply H3.
Qed.
(** [] *)
...
Proof.
  ...
  - induction re.
    ...
    + simpl. intros [s H]. inversion H.
      * assert (Hre1: re_not_empty re1 = true).
        ...
        rewrite -> Hre1. reflexivity.
      * assert (Hre2: re_not_empty re2 = true).
        { apply IHre2. exists s. apply H2. }
        rewrite -> IHre2.
  ...
  - induction re.
    ...
    + intro H. exists []. apply MEmpty.
    + intro H. exists [t]. apply MChar.
    + intro H. simpl in H.
      destruct (re_not_empty re1).
    ...
    + intros H. simpl in H.
      destruct (re_not_empty re1).
      * simpl in H. apply IHre1 in H.
        destruct H as [s H].
        exists s.
        apply MUnionL.
        apply H.
      * { destruct (re_not_empty re2).
          - simpl in H. apply IHre2 in H.
            destruct H as [s H].
            exists s.
            apply MUnionR.
            apply H.
...
Proof.
  ...
  - inversion Heqre'.
    ...
    destruct Heqre' as [ss' [Hs5 Hss']].
    exists ([s4] ++ ss').
    split.
    + simpl. rewrite <- Hs5. reflexivity.
    + simpl. intros s' [HIn1 | HIn2].
      * rewrite <- HIn1. rewrite <- H2. apply H.
...
Proof.
  ...
  - (* MApp *)
    ...
    intros Hlen. apply Nat.add_le_cases in Hlen.
    destruct Hlen as [Hlen1 | Hlen2].
    + apply IH1 in Hlen1.
      destruct Hlen1 as [s0' [s4' [s5' [H1 [H2 H3]]]]].
      exists s0'. exists s4'. exists (s5' ++ s2).
      split.
      * rewrite -> H1.
        assert (Hassoc1: (s0' ++ s4' ++ s5') ++ s2 = s0' ++ (s4' ++ s5') ++ s2).
        { rewrite <- app_assoc. reflexivity. }
        assert (Hassoc2: (s4' ++ s5') ++ s2 = s4' ++ s5' ++ s2).
        { rewrite <- app_assoc. reflexivity. }
        rewrite -> Hassoc1. rewrite -> Hassoc2. reflexivity.
      * { split.
          - apply H2.
          - intro m.
            assert (Hassoc1: napp m s4' ++ s5' ++ s2
                             = (napp m s4' ++ s5') ++ s2).
            { rewrite -> app_assoc. reflexivity. }
            assert (Hassoc2: s0' ++ (napp m s4' ++ s5') ++ s2
                             = (s0' ++ napp m s4' ++ s5') ++ s2).
            { rewrite -> app_assoc. reflexivity. }
            rewrite -> Hassoc1. rewrite -> Hassoc2.
            apply MApp. apply H3. apply Hmatch2. }
    + apply IH2 in Hlen2.
      destruct Hlen2 as [s0' [s4' [s5' [H1 [H2 H3]]]]].
      exists (s1 ++ s0'). exists s4'. exists s5'.
      split.
  ...
  - simpl. intro Hlen.
    ...
    { apply le_plus_l. }
    apply (le_trans (pumping_constant re1)
                    (pumping_constant re1 + pumping_constant re2)
                    (length s1)) in H.
    apply IH in H.
  ...
  - simpl. intro Hlen.
    ...
    { apply (le_trans (pumping_constant re2)
                    (pumping_constant re1 + pumping_constant re2)
                    (length s2)) in H.
      apply H. apply Hlen. }
    apply IH in H0.
    destruct H0 as [s4 [s0 [s5 [H1 [H2 H3]]]]].
    exists s4. exists s0. exists s5.
    split.
    + apply H1.
    + split.
      * apply H2.
  ...
  - simpl. intro H. rewrite -> app_length in H.
    destruct s1 as [| n1 s1'].
    + destruct s2 as [| n2 s2'].
      * simpl in H. omega.
      * simpl in H. simpl in IH2.
        apply IH2 in H.
        destruct H as [s1 [s2 [s3 [H1 [H2 H3]]]]].
        ...
        { split.
          ...
          - split.
            + apply H2.
            + intro m. apply H3. }
    + { exists []. exists (n1 :: s1'). exists s2.
        split.
        - simpl. reflexivity.
        - split.
          + intro contra. inversion contra.
          + intro m. simpl. apply star_app.
            { induction m as [| m' IH].
              - simpl. apply MStar0.
              - remember (n1 :: s1') as s1.
...
Proof.
  ...
  - inversion H. split.
    ...
    + intros. apply H0.
  - inversion H. split. 
    + intros HP. apply H0 in HP. inversion HP.
...
Lemma strong_induction : forall (P : nat -> Prop),
                           ...
                           forall n, P n.
Proof.
  intros P H n. apply H.
  induction n as [| n' IH].
  - intros m contra. inversion contra.
  - assert (Hn': P n').
    { apply H in IH. apply IH. }
    intros m H'.
    inversion H'.
    + apply Hn'.
    + apply IH. unfold lt. apply H1.
Qed.
...
Proof.
  ...
  - destruct n' as [| n''].
    + apply H1.
    + assert (SS_plus: S (S n'') = 2 + n'').
      { reflexivity. }
      rewrite -> SS_plus. rewrite -> plus_comm.
      apply H2. apply IH.
      unfold lt. apply le_S. apply le_n.
Qed.

Lemma len_induction : forall X (P : list X -> Prop),
...
Lemma list_length_SS : forall X (l : list X) n,
                         (length l = n + 2) ->
                         exists l' x y,
                           l = x :: l' ++ [y] /\ length l' = pred (pred (length l)).
Proof.
  intros X l n H.
  ...
  destruct l as [| x l'].
  - simpl in H. inversion H.
  - destruct (rev l') as [| y l''] eqn:Hrev.
    + assert (Hl: l' = []).
      { destruct l' as [| x' l''].
        - reflexivity.
        - assert (Hrev': x' :: l'' = []).
          { rewrite <- rev_involutive with (l := x' :: l'').
            rewrite -> Hrev. reflexivity. }
          inversion Hrev'. }
      rewrite -> Hl in H. simpl in H. inversion H.
    + assert (Hl: l' = rev l'' ++ [y]).
      { rewrite <- rev_involutive with (l := l').
        rewrite -> Hrev. reflexivity. }
      exists (rev l''). exists x. exists y. split.
...
Lemma pal_induction : forall (X : Type) (P : list X -> Prop),
                        P [] ->
                        (forall x, P [x]) ->
                        (forall l x y, P l -> P (x :: l ++ [y])) ->
                        (forall l, P l).
Proof.
  intros X P H0 H1 H2.
  apply len_induction.
  ...
  assert (Pn0: Pn 0).
  { rewrite -> HeqPn. intros l.
    destruct l as [| x l'].
    - simpl. intros _. apply H0.
  ...
  { rewrite -> HeqPn. intros l.
    ...
    - destruct l' as [| x' l''].
      + intros _. apply H1.
      + simpl. intro contra. inversion contra. }
  assert (Pn2: forall n, Pn n -> Pn (n + 2)).
  { rewrite -> HeqPn.
    intros n H l H'.
    remember H' as Hlen. clear HeqHlen.
    apply list_length_SS in H'.
    destruct H' as [l' [x [y [Hl1 Hl2]]]].
    rewrite -> Hlen in Hl2. rewrite -> plus_comm in Hl2.
    simpl in Hl2. apply H in Hl2.
    rewrite -> Hl1. apply H2. apply Hl2. }
  intro n.
  assert (Result: Pn n).
  { apply nat_induction.
    apply Pn0. apply Pn1. apply Pn2. }
  rewrite -> HeqPn in Result.
  apply Result.
Qed.

Lemma rev_cons : forall (X : Type) (x : X) (l : list X),
                   rev (l ++ [x]) = x :: (rev l).
Proof.
  intros X x l.
  ...
  - reflexivity.
  - simpl. rewrite -> IH.
    replace ((x :: rev l') ++ [x']) with (x :: rev l' ++ [x']).
    reflexivity.
    reflexivity.
Qed.
...
Proof.
  ...
  - intros l0 x y H H'.
    ...
    rewrite -> rev_involutive. rewrite <- H4. reflexivity.
    reflexivity.
Qed.
...
| merge0 : merge [] [] []
| mergel : forall l1 l2 l x, merge l1 l2 l -> merge (x::l1) l2 (x::l)
| merger : forall l1 l2 l x, merge l1 l2 l -> merge l1 (x::l2) (x::l).

Theorem merge_filter : forall X (l1 l2 l : list X) (test : X -> bool),
                         merge l1 l2 l ->
...
Proof.
  ...
  - simpl.
    ...
    + assert (H: filter test l = l1).
      { apply IHHm. intros x0 H.
        apply H1. simpl. right. apply H.
        apply H2. }
...
Theorem filter_subseq : forall X (l l' : list X) (test : X -> bool),
                          subseq l' l ->
                          (forall x, In x l' -> test x = true) ->
                          length l' <= length (filter test l).
Proof.
  intros X l l' test Hs Htest.
  induction Hs.
  - reflexivity.
  - apply IHHs in Htest.
    apply (le_trans (length l1) (length (filter test l2)) (length (filter test (n :: l2)))).
    apply Htest.
    simpl. destruct (test n).
    + simpl. apply le_S. apply le_n.
    + apply le_n.
  - simpl. destruct (test n) eqn:Hn.
    + simpl. apply le_n_S. apply IHHs.
      intros x H.
      apply Htest. simpl. right. apply H.
    + assert (contra: test n = true).
      { apply Htest. simpl. left. reflexivity. }
      rewrite -> Hn in contra. inversion contra.
Qed.
(** [] *)
...
Proof.
  ...
  induction l1 as [| x' l1' IH].
  - simpl. apply H2.
  - simpl. apply NoDup1. split.
    + apply IH.
      * simpl in HDis. destruct HDis as [Hdis1 Hdis2]. apply Hdis2.
      * inversion H1. destruct H0 as [H0 _]. apply H0.
    + intro contra. rewrite -> in_app_iff in contra.
      destruct contra as [contra | contra].
      * inversion H1. destruct H0 as [_ H0].
        apply H0 in contra. apply contra.
      * simpl in HDis. destruct HDis as [HDis _].
        apply HDis in contra. apply contra.
Qed.
(** [] *)
...
Definition is_not_head {X} (l : list X) (x : X) : Prop :=
  match l with
    | [] => True
    | h :: t => h <> x
  end.
...
| nostutter1 : forall l x, nostutter l /\ is_not_head l x -> nostutter (x :: l).
...
| repeats1 : forall l x, In x l -> repeats (x :: l).
