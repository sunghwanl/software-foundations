...
Proof.
  ...
  - apply (WHILE_true_nonterm _ c st st') in HB.
    apply HB in H. inversion H.
  - apply (WHILE_true_nonterm BTrue SKIP st st') in H.
    + inversion H.
    + intros st''. unfold beval. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - replace st' with (t_update st' X (aeval st' e)).
    + inversion H. subst. apply E_Ass. reflexivity.
    + apply functional_extensionality. intro.
      unfold aequiv in Ha. rewrite <- Ha.
      rewrite t_update_same. reflexivity.
  - inversion H; subst; simpl.
...
Proof.
  ...
  - remember (c1 ;; c2) as cseq eqn:Heqcseq.
    induction Hc; inversion Heqcseq; subst.
    apply E_Seq with (st' := st').
    rewrite <- Hc1. assumption.
    rewrite <- Hc2. assumption.
  - remember (c1' ;; c2') as cseq eqn:Heqcseq.
    induction Hc; inversion Heqcseq; subst.
    apply E_Seq with (st' := st').
    rewrite -> Hc1. assumption.
...
Proof.
  ...
  - destruct a1; try (rewrite IHa1; rewrite IHa2; reflexivity).
    simpl in IHa1, IHa2. inversion IHa1.
    simpl. rewrite IHa2.
...
Proof.
  unfold ctrans_sound. intros c. unfold cequiv.
  induction c; simpl; try reflexivity;
  split; intros H; inversion H; subst.
  - apply E_Ass. rewrite <- optimize_0plus_aexp_sound. reflexivity.
  - apply E_Ass. rewrite <- optimize_0plus_aexp_sound. reflexivity.
  ...
  - apply E_Seq with (st' := st'0).
    + rewrite -> IHc1. assumption.
    + rewrite -> IHc2. assumption.
  - apply E_IfTrue. rewrite <- optimize_0plus_bexp_sound. assumption.
    rewrite <- IHc1. assumption.
  -  apply E_IfFalse. rewrite <- optimize_0plus_bexp_sound. assumption.
    rewrite <- IHc2. assumption.
  - apply E_IfTrue. rewrite -> optimize_0plus_bexp_sound. assumption.
    rewrite -> IHc1. assumption.
  -  apply E_IfFalse. rewrite -> optimize_0plus_bexp_sound. assumption.
  ...
  - apply E_WhileLoop with (st' := st'0).
    ...
    + assert (Hbeq: bequiv b (optimize_0plus_bexp b)).
      { apply optimize_0plus_bexp_sound. }
      assert (Hceq: cequiv c (optimize_0plus_com c)).
      { unfold cequiv. apply IHc. }
      assert (Heq: cequiv (WHILE b DO c END)
  ...
  - apply E_WhileLoop with (st' := st'0).
    + rewrite -> optimize_0plus_bexp_sound. assumption.
    + rewrite -> IHc. assumption.
    + assert (Hbeq: bequiv (optimize_0plus_bexp b) b).
      { apply sym_bequiv. apply optimize_0plus_bexp_sound. }
      assert (Hceq: cequiv (optimize_0plus_com c) c).
      { unfold cequiv. intros. rewrite -> IHc. reflexivity. }
      assert (Heq: cequiv (WHILE optimize_0plus_bexp b
                                 DO optimize_0plus_com c END)
                          (WHILE b DO c END)).
      { apply CWhile_congruence; assumption. }
      apply Heq. assumption.
...
Proof.
  intros i st a ni H.
  induction a; simpl; try reflexivity; inversion H; subst;
...
Definition subst_equiv_property' := forall i1 i2 a1 a2,
                                 ...
                                 cequiv (i1 ::= a1 ;; i2 ::= a2)
                                        (i1 ::= a1 ;;
                                            i2 ::= subst_aexp i1 a1 a2).

Theorem subst_sound_if_vnu : subst_equiv_property'.
Proof.
  intros i1 i2 a1 a2 Hvnu. unfold cequiv. intros.
  split; intros H; inversion H; subst.
  - apply E_Seq with (st' := st'0). assumption.
    inversion H2; subst; inversion H5; subst.
    apply E_Ass. clear H2 H H5.
    induction a2; simpl; try reflexivity;
    try (rewrite IHa2_1; rewrite IHa2_2; reflexivity).
    + destruct (beq_id i1 i) eqn:Heq.
      * rewrite beq_id_true_iff in Heq. subst.
        assert (H: aeval (t_update st i (aeval st a1)) a1 = aeval st a1).
        { apply aeval_weakening. assumption. }
        rewrite -> H. unfold t_update. rewrite <- beq_id_refl. reflexivity.
      * simpl. reflexivity.
  - apply E_Seq with (st' := st'0). assumption.
    inversion H2; subst; inversion H5; subst.
    apply E_Ass. clear H2 H H5.
...
Proof.
  intros H.
  assert (HSkip: forall st, SKIP / st \\ st) by (apply E_Skip).
  unfold cequiv in H.
  assert (contra: (WHILE BTrue DO SKIP END) / (t_empty 0) \\ (t_empty 0)).
  { rewrite -> H. apply HSkip. }
...
Proof.
  left.
  destruct (beq_id X Y) eqn:HeqXY.
  - inversion HeqXY.
  - unfold cequiv, pXY, pYX. intros st st'.
    split; intros H; inversion H; subst;
    inversion H2; subst; inversion H5; subst;
    rename n into n1; rename n0 into n2;
    rewrite beq_id_false_iff in HeqXY.
    + apply (t_update_permute _ n2 n1 Y X st) in HeqXY.
      rewrite HeqXY.
      apply E_Seq with (st' := t_update st Y n2); apply E_Havoc.
    + apply (t_update_permute _ n1 n2 Y X st) in HeqXY.
      rewrite <- HeqXY.
      apply E_Seq with (st' := t_update st X n2); apply E_Havoc.
Qed.
(** [] *)
...
Proof.
  right.
  assert (Hptwice: ptwice / empty_state
                          \\ t_update (t_update empty_state X 0) Y 1).
  { apply E_Seq with (st' := t_update empty_state X 0); apply E_Havoc. }
  unfold cequiv. intros contra.
  assert (Hpcopy: pcopy / empty_state
                        \\ t_update (t_update empty_state X 0) Y 1).
  { rewrite <- contra. apply Hptwice. }
  unfold pcopy in Hpcopy.
  inversion Hpcopy; subst; inversion H1; subst; inversion H4; subst.
  simpl in H5. rewrite t_update_eq in H5.
  clear Hptwice contra Hpcopy H1 H4.
  destruct (beq_nat n 0) eqn:Heqn.
  - rewrite beq_nat_true_iff in Heqn. rewrite Heqn in H5.
    assert (t_update (t_update empty_state X 0) Y 0 Y =
            t_update (t_update empty_state X 0) Y 1 Y)
      by (rewrite H5; reflexivity).
    repeat rewrite t_update_eq in H. inversion H.
  - rewrite beq_nat_false_iff in Heqn.
    destruct (beq_id X Y) eqn:HeqXY.
    + rewrite beq_id_true_iff in HeqXY. inversion HeqXY.
    + rewrite beq_id_false_iff in HeqXY.
      assert (HXn: t_update (t_update empty_state X n) Y n =
                   t_update (t_update empty_state Y n) X n)
        by (apply t_update_permute; apply HeqXY).
      assert (HX0: t_update (t_update empty_state X 0) Y 1 =
                   t_update (t_update empty_state Y 1) X 0)
        by (apply t_update_permute; apply HeqXY).
      rewrite HXn in H5. rewrite HX0 in H5.
      assert (t_update (t_update empty_state Y n) X n X =
              t_update (t_update empty_state Y 1) X 0 X)
        by (rewrite H5; reflexivity).
      repeat rewrite t_update_eq in H.
      apply Heqn in H. apply H.
Qed.
(** [] *)
...
Proof.
  ...
  - rewrite <- beq_nat_false_iff in Hx.
    simpl in H. rewrite Hx in H. inversion H.
  - apply IHceval2.
    + clear IHceval1 H Heqc H1 IHceval2.
      inversion H0; subst; inversion H2; subst; inversion H5; subst.
      clear H2 H0 H5. simpl.
      assert (t_update (t_update st Y n) X
                       (t_update st Y n X + 1) X =
              t_update st Y n X + 1).
      { apply t_update_eq. }
      rewrite -> H. intros contra.
      rewrite plus_comm in contra. inversion contra.
...
Proof.
  ...
  split; intros H.
  - destruct (beq_nat (st X) 0) eqn:Hx.
    + rewrite beq_nat_true_iff in Hx.
      inversion H; subst.
      * apply E_WhileEnd. apply H4.
      * simpl in H2. rewrite Hx in H2. inversion H2.
    + rewrite beq_nat_false_iff in Hx.
      inversion H; subst.
      * apply (p1_may_diverge st' st' Hx) in H. inversion H.
      * apply (p1_may_diverge st st' Hx) in H. inversion H.
  - destruct (beq_nat (st X) 0) eqn:Hx.
    + rewrite beq_nat_true_iff in Hx.
      inversion H; subst.
    ...
    + rewrite beq_nat_false_iff in Hx.
      inversion H; subst.
      * apply (p2_may_diverge st' st' Hx) in H. inversion H.
      * apply (p2_may_diverge st st' Hx) in H. inversion H.
Qed.
(** [] *)
...
Lemma p3_eval : p3 / t_update empty_state X 1 \\
                   t_update (t_update empty_state X 0) Z 0.
Proof.
  assert (St: t_update (t_update (t_update
                                    (t_update empty_state X 1) Z 1) X 0) Z 0 =
              t_update (t_update empty_state X 0) Z 0).
  { apply functional_extensionality. intros x.
    destruct (beq_id Z x) eqn:Hx1.
    - unfold t_update. rewrite Hx1. reflexivity.
    - destruct (beq_id X x) eqn:Hx2.
      + unfold t_update. rewrite Hx1. rewrite Hx2. reflexivity.
      + unfold t_update. rewrite Hx1. rewrite Hx2. reflexivity. }
  rewrite <- St. clear St.
  apply E_Seq with (st' := t_update (t_update empty_state X 1) Z 1).
  - apply E_Ass. reflexivity.
  - apply E_WhileLoop with (st' := t_update (t_update (t_update (t_update empty_state X 1) Z 1) X 0) Z 0).
    + reflexivity.
    + apply E_Seq with (st' := (t_update (t_update (t_update empty_state X 1) Z 1) X 0)).
      * apply E_Havoc.
      * apply E_Havoc.
...
Proof.
  intros ceq. unfold cequiv in ceq.
  assert (H: p3 / t_update empty_state X 1 \\
                t_update (t_update empty_state X 0) Z 0)
    by (apply p3_eval).
  rewrite ceq in H.
  inversion H; subst.
  inversion H2; subst.
  clear H H2. simpl in H5.
  assert (St: t_update (t_update empty_state X 1) X 0 =
              t_update empty_state X 0)
    by (apply t_update_shadow).
  rewrite St in H5. inversion H5; subst. simpl in H3.
  assert (contra: t_update (t_update empty_state X 0) Z 1 Z =
                  t_update (t_update empty_state X 0) Z 0 Z)
...
Lemma p5_eval : forall st st', p5 / st \\ st' <-> st' = t_update st X 1.
Proof.
  intros st st'. split; intros H.
  - remember p5 as c.
    induction H; inversion Heqc; subst.
    + simpl in H.
      assert (HX: negb (negb (beq_nat (st X) 1)) = true)
        by (rewrite H; reflexivity).
      rewrite negb_involutive in HX.
      rewrite beq_nat_true_iff in HX.
      apply functional_extensionality.
      intros x. clear Heqc H.
      destruct (beq_id X x) eqn:Heq.
      * unfold t_update. rewrite Heq.
        rewrite beq_id_true_iff in Heq. rewrite <- Heq. apply HX.
      * unfold t_update. rewrite Heq. reflexivity.
    + assert (Hst: t_update st X 1 = t_update st' X 1).
      { clear IHceval1 H Heqc H1 IHceval2.
        inversion H0; subst.
        rewrite t_update_shadow. reflexivity. }
      rewrite <- Hst in IHceval2.
      apply IHceval2.
      apply Heqc.
  - rewrite H. clear H.
    destruct (beq_nat (st X) 1) eqn:HeqX.
    + assert (H: t_update st X 1 = st).
      { apply functional_extensionality.
        intros x.
        destruct (beq_id X x) eqn:Heqx.
        - unfold t_update. rewrite Heqx.
          rewrite beq_nat_true_iff in HeqX.
          rewrite beq_id_true_iff in Heqx.
          rewrite <- Heqx. rewrite HeqX. reflexivity.
        - unfold t_update. rewrite Heqx. reflexivity. }
      rewrite H. apply E_WhileEnd.
      simpl. rewrite HeqX. reflexivity.
    + apply E_WhileLoop with (st' := t_update st X 1).
      * simpl. rewrite HeqX. reflexivity.
      * apply E_Havoc.
      * apply E_WhileEnd. reflexivity.
Qed.

Lemma p6_eval : forall st st', p6 / st \\ st' <-> st' = t_update st X 1.
Proof.
  intros st st'. split; intros H.
  - inversion H; subst. reflexivity.
  - rewrite H. clear H. apply E_Ass. reflexivity.
Qed.

Theorem p5_p6_equiv : cequiv p5 p6.
Proof.
  intros st st'. split; intros H.
  - rewrite p5_eval in H. rewrite p6_eval. apply H.
  - rewrite p6_eval in H. rewrite p5_eval. apply H.
Qed.
(** [] *)
...
Lemma noninterfering_state: forall st l1 l2 a1 a2,
  l1 <> l2 ->
...
Proof.
  intros st l1 l2 a1 a2 Hl H1 H2.
  apply functional_extensionality.
  intros x.
  destruct (beq_id l1 x) eqn:Heql1.
  - assert (Heql2: beq_id l2 x = false).
    { rewrite beq_id_true_iff in Heql1.
      rewrite Heql1 in Hl.
      rewrite beq_id_false_iff.
      intros contra. apply Hl. rewrite contra. reflexivity. }
    unfold t_update. rewrite Heql1. rewrite Heql2.
    apply (aeval_weakening l2 st a1 (aeval st a2)) in H2.
    rewrite <- H2. reflexivity.
  - destruct (beq_id l2 x) eqn:Heql2.
    + unfold t_update. rewrite Heql1. rewrite Heql2.
      apply (aeval_weakening l1 st a2 (aeval st a1)) in H1.
      rewrite <- H1. reflexivity.
    + unfold t_update. rewrite Heql1. rewrite Heql2. reflexivity.
Qed.
...
Theorem swap_noninterfering_assignments: forall l1 l2 a1 a2,
  ...
  cequiv
    ...
    (l2 ::= a2;; l1 ::= a1).
Proof.
  intros l1 l2 a1 a2 Hl H1 H2 st st'.
  split; intros H.
  - inversion H; subst; inversion H4; subst; inversion H7; subst.
    rewrite noninterfering_state; try assumption.
    apply E_Seq with (st' := t_update st l2 (aeval st a2));
      (apply E_Ass; reflexivity).
  - inversion H; subst; inversion H4; subst; inversion H7; subst.
    rewrite noninterfering_state; try assumption.
    apply E_Seq with (st' := t_update st l1 (aeval st a1));
      (apply E_Ass; reflexivity).
    intros contra. apply Hl. rewrite contra. reflexivity.
Qed.
(** [] *)
